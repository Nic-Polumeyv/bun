...
/// handle directly.
pub(super) unsafe extern "C" fn on_new_conn(
    owner: *mut c_void,
...
pub(super) unsafe extern "C" fn on_dg_write(
    ctx: *mut c_void,
    buf: *mut c_void,
    sz: usize,
) -> isize {
...
pub(super) unsafe extern "C" fn on_origin(
    ctx: *mut c_void,
    chunk: *const u8,
    len: usize,
    fin: c_int,
...
pub(super) unsafe extern "C" fn on_datagram(ctx: *mut c_void, buf: *const c_void, sz: usize) {
    ...
    let early =
        unsafe { lsquic::Conn::from_raw(session.conn.get()) }.is_some_and(|c| c.datagram_early());
